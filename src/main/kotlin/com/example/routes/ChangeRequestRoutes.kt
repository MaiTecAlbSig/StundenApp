...
fun Route.changeRequestRoutes(changeRequestDao: ChangeRequestDao) {
    route("/change-request"){
        ...
        post {
            ...
            val requestDescription = params["request_description"] ?: return@post call.respond(HttpStatusCode.BadRequest, "Request description required")
            val currentDate = LocalDate.now().format(DateTimeFormatter.ISO_LOCAL_DATE)

            val article = changeRequestDao.addChangeRequest(projectId.toInt(), employeeId.toInt(), requestType, requestDescription, currentDate)
        ...
        get("/status/{status}"){
            val status = call.parameters["status"] ?: return@get call.respond(HttpStatusCode.BadRequest, "Status required")
