    ...
    authenticate("auth-jwt") {
        route("/hours") {
            post {
                ...
                val hour_worked =
                    params["hours_worked"] ?: return@post call.respond(HttpStatusCode.BadRequest, "Hour required")
                val date = params["date"] ?: return@post call.respond(HttpStatusCode.BadRequest, "Day required")
                val description =
                    params["description"] ?: return@post call.respond(HttpStatusCode.BadRequest, "Description required")

                val article =
                    hourDao.addHour(projectId.toInt(), employeeId.toInt(), hour_worked.toDouble(), date, description)
